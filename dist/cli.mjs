...
async function handleCodeMetrics(args) {
  const targetDir = path2.resolve(process.cwd(), args.dirPath || ".");
  ...
  function walk(currentDir) {
    const files = fs3.readdirSync(currentDir, { withFileTypes: true });
    for (const file of files) {
      if (file.name.startsWith(".") || file.name === "node_modules" || file.name === "dist" || file.name === "build") {
        continue;
      }
      const fullPath = path2.join(currentDir, file.name);
      if (file.isDirectory()) {
        walk(fullPath);
      } else if (file.isFile()) {
        const ext = path2.extname(file.name).toLowerCase() || "no-extension";
        try {
